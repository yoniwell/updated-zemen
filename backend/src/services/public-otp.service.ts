...
const readOtpRecord = async (purpose: PortalOtpPurpose, email: string): Promise<OtpRecord | null> => {
  const setting = await prisma.systemSetting.findUnique({ where: { key: otpKey(purpose, email) } });
  if (!setting) {
    return null;
  }
...
export async function issuePortalOtp(input: {
  ...
  await prisma.systemSetting.upsert({
    where: { key: otpKey(input.purpose, email) },
    create: {
      key: otpKey(input.purpose, email),
...
export async function verifyPortalOtpCode(input: {
  ...
  if (new Date(otp.expiresAt).getTime() < now) {
    await prisma.systemSetting.deleteMany({ where: { key: otpKey(input.purpose, email) } });
  ...
  await prisma.systemSetting.create({
    data: {
      key: verifiedKey(input.purpose, token),
      value: JSON.stringify({
        email,
...
export async function consumePortalOtpVerificationToken(input: {
  ...
  const email = normalizeEmail(input.email);
  const key = verifiedKey(input.purpose, input.verificationToken);
  const setting = await prisma.systemSetting.findUnique({ where: { key } });

  if (!setting) {
    throw new PortalOtpError('OTP verification is required before submitting.', 401);
  }

  let parsed: VerifiedRecord;
  try {
    parsed = JSON.parse(setting.value) as VerifiedRecord;
  } catch {
    await prisma.systemSetting.deleteMany({ where: { key } });
    throw new PortalOtpError('OTP verification is invalid. Please verify again.', 401);
  }

  if (parsed.purpose !== input.purpose || normalizeEmail(parsed.email) !== email) {
    await prisma.systemSetting.deleteMany({ where: { key } });
    throw new PortalOtpError('OTP verification does not match this email.', 401);
  }

  if (new Date(parsed.expiresAt).getTime() < Date.now()) {
    await prisma.systemSetting.deleteMany({ where: { key } });
    throw new PortalOtpError('OTP verification expired. Please verify again.', 401);
  }

...
import { auditIntegrityJobRepository } from '../repositories/audit-integrity-job.repository';
...
const publishIntegrityAlert = async (result: {
  ...
  await auditIntegrityJobRepository.createNotificationEvent({
    status: 'WARNING',
...
const runAuditIntegrityCheck = async (): Promise<void> => {
  try {
    const result = await verifyAuditIntegrity() as any;
    if (!result.verified && !result.valid) { // handle both possible return keys
      await publishIntegrityAlert(result);
      console.error('Audit integrity verification failed', {
        message: result.message,
        totalLogs: result.totalLogs,
        anchorCount: result.anchoredLogs || result.anchorCount,
      });
      return;
    }
